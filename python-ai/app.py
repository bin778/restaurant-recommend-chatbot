    ...
    try:
        ...
        if not search_results or not search_results.get("items"):
            ...
            **[지시사항]**
            ...
            3. '검색된 맛집 정보'를 바탕으로, 추천할 만한 가게들을 **번호를 매긴 목록(Numbered List)**으로 설명해줘. (예: 1. A식당, 2. B식당)
            4. 각 가게를 소개할 때는, **줄 바꿈(\\n)을 사용해서 문단을 명확하게 나누어줘.** 가독성이 매우 중요해.
            5. 가게 이름은 그냥 텍스트로 말해줘. **절대 `**` 같은 마크다운 문법을 사용하지 마.**
            6. 각 가게의 특징(카테고리)과 위치(주소)를 명확히 언급해줘.
            7. 절대 '검색된 맛집 정보'에 없는 내용은 지어내서 말하면 안돼.
            8. 마지막에는 친근한 마무리 인사를 덧붙여줘.
            ...
            **[추천 답변]**
            서울 곱창 맛집을 찾으시는군요! 검색 결과를 바탕으로 몇 곳 추천해 드릴게요.

            1. 싹쓰리곱창 성수본점
            성수동에 위치한 한식, 특히 곱창, 막창, 양 전문점입니다. (주소: 서울특별시 성동구 성수동2가 331-85 1층)

            2. 도화곱창
            마포구 도화동에 위치한 곱창, 막창, 양을 전문으로 하는 한식집입니다. (주소: 서울특별시 마포구 도화동 179-7 1층)

            이 정보가 맛집을 고르시는 데 도움이 되기를 바랍니다!
            """
