...
- **원인 분석**: `mkcert -install`로 생성된 루트 인증 기관(CA)은 **명령어를 실행한 로컬 머신(맥북)에만 자동으로 신뢰하도록 등록**됨. 다른 기기들은 이 사설 인증 기관을 알지 못하므로, 해당 기관이 발급한 SSL 인증서를 신뢰할 수 없는 것으로 판단하여 연결을 거부함.
