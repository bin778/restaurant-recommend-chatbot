...
| C-06        | 음성 인식   | 사용자가 음성으로 대답할 때도 인식하기.               |  **중**  | 구현 예정             |
| **Admin**   |
...
이 프로젝트를 진행하며 마주했던 주요 기술적 고민과 해결 과정
...
-   **해결 과정**: 응답 인터셉터 로직에 `/api/auth/login` 경로로 요청 시 발생한 에러는 재발급 로직을 실행하지 않도록 예외 처리하여 무한 루프의 고리를 끊음.

#### 6. 타 기기에서의 로그인 요청 실패 (진행 중)

-   **문제 현상**: 데스크톱에서는 HTTPS 환경에서 정상적으로 로그인이 되지만, 동일한 Wi-Fi에 연결된 모바일 기기에서는 여전히 로그인 요청이 실패하는 문제.
-   **현재 상태**: 백엔드와 프론트엔드 모두 HTTPS가 적용되었고, CORS 및 API 요청 주소도 올바르게 설정된 것을 확인. 네트워크 레벨의 다른 문제로 추정되며, 현재 원인 파악 및 해결 진행 중.
